...
public class getReuest
{
    ...
    //    System.Net.HttpWebRequest req = new HttpWebRequest();
    //}
    ...
    //POST方法(WebClient)
    public static string GetHttp(string url,bool useProxy = true,string proxyadd="",int port=80)
    {
        HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);
        httpWebRequest.ContentType = "application/json";
        httpWebRequest.Method = "GET";
        ...
        if (useProxy)
        {
            WebProxy myProxy = new WebProxy(proxyadd,port); 
