...
{ 
    public void load(string path = @"D:\WorkSpace-old\NewWorkVs10\TimeRace_Financial-IQ\1\trunk\TimeRace_Financial-IQ\LibraryFactory\myDllFactory.dll")
    {
        Byte[] byte1 = System.IO.File.ReadAllBytes(path);//也是可以的
        Assembly _assem = Assembly.Load(byte1); 
        ...
        try
        {
            string t_class = "myNamespace.PubClass";
            t_class = "myNamespace.PubClass,myDllFactory.dll";
            t_class = "myNamespace.PubClass,myDllFactory";
        ...
        foreach (Type ty in tys)//huoquleiming
        {
            ...
            if (ty.Name == "PriClass")
            {
                ...
                MethodInfo mi = ty.GetMethod("ShowName");
                object aa = mi.Invoke(magicClassObject, new object[] { ty }); 
            }
            if (ty.Name == "cc")
            {
                object magicClassObject = Activator.CreateInstance(ty);
                MethodInfo mi = ty.GetMethod("GetCC");
                object aa = mi.Invoke(magicClassObject, new object[] { "haha!" }); 
            }
            if (ty.Name == "cp")
            { 
                ConstructorInfo magicConstructor = ty.GetConstructor(Type.EmptyTypes);//获取不带参数的构造函数
